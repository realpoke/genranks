<x-cards.option>
    ...
    <div class="flex items-center mt-5">
        <x-buttons.main wire:click="confirmLogout" wire:loading.attr="disabled">
            {{ __('Log Out Other Browser Sessions') }}
        </x-buttons.main>

        <x-sections.action-message class="ml-3" on="logged-out">
            {{ __('Done.') }}
        </x-sections.action-message>
    </div>

    <!-- Log Out Other Devices Confirmation Modal -->
    <x-modals.dialog wire:model="form.confirmingLogout">
        <x-slot name="title">
            {{ __('Log Out Other Browser Sessions') }}
        </x-slot>
        ...
        <x-slot name="content">
            ...
            <div class="mt-4" x-data="{}"
                ...
                <x-inputs.text type="password" class="block w-3/4 mt-1" placeholder="{{ __('Password') }}"
                    x-ref="password" wire:model="form.password" wire:keydown.enter="logoutSessions" />
        ...
        <x-slot name="footer">
            <x-buttons.secondary wire:click="$toggle('form.confirmingLogout')" wire:loading.attr="disabled">
                {{ __('Cancel') }}
            </x-buttons.secondary>

            <x-buttons.main class="ml-3" wire:click="logoutSessions" wire:loading.attr="disabled">
                {{ __('Log Out Other Browser Sessions') }}
            </x-buttons.main>
        </x-slot>
    </x-modals.dialog>
